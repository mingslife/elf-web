  ...
  <div>
    ...
    >
      ...
      <form ref="form" @submit.stop.prevent="save">
        ...
        >
          <b-form-input
            v-model="record.categoryName" type="text"
            minlength="2" maxlength="50" required trim
            :placeholder="$t('common.required')"
            :state="$validity['categoryName']"
            @input.capture="$checkValidity('categoryName', $event)"
            :disabled="disabled"
          ></b-form-input>
        </b-form-group>
        ...
        <b-form-group>
          <b-form-checkbox
            ...
            :disabled="disabled"
          >
            <span v-text="$t('category.isPrivate')"></span>
          </b-form-checkbox>
        </b-form-group>
      </form>
    </b-modal>
...
export default {
  ...
  data () {
    return {
      /** 基础部分 */
      fields: [
        { key: 'id', label: this.$t('category.id') },
        { key: 'categoryName', label: this.$t('category.categoryName') },
        { key: 'route', label: this.$t('category.route') },
        {
          key: 'isPrivate',
          label: this.$t('category.isPrivate'),
        ...
        {
          key: 'updatedAt',
          label: this.$t('category.updatedAt'),
          formatter: (v) => v ? dayjs(v).format('YYYY-MM-DD HH:mm:ss') : ''
        },
  ...
  methods: {
    ...
    listRecords (limit, page) {
      return this.$axios.get(`/categories?limit=${limit}&page=${page}`)
    },
    // 获取记录
    getRecord (id) {
    ...
    // 创建记录
    createRecord (record) {
      return this.$axios.post('/categories', this.formatRecord(record))
    },
    // 更新记录
    updateRecord (id, record) {
      return this.$axios.put(`/categories/${id}`, this.formatRecord(record))
    },
    // 删除记录
    deleteRecord (id) {
